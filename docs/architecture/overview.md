...
| @repo/core Package | Domain services layer | Per-domain services (projects, tags, snapshots, ...), schema definitions, database operations |
...
The PostgreSQL database is accessed through the `@repo/core` package, which provides database connection handling, schema definitions, and the domain services built on top of them. Each table is defined in a `<name>.sql.ts` file inside the folder of the service that owns it (`packages/core/src/services/tags/tags.sql.ts`), and `packages/core/src/schema.ts` re-exports all of them.

Sources: [packages/core/package.json](../../packages/core/package.json) [packages/core/drizzle/meta/0000\_snapshot.json](../../packages/core/drizzle/meta/0000_snapshot.json)
