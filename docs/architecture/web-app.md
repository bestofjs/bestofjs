...
| Source | Surfaces |
| --- | --- |
| PostgreSQL, via `@repo/core` queries wrapped in `"use cache"` | Home page (hot lists, recently added, popular tags, catalog stats), `/trends/*`, `/projects` and its detail pages (NPM dependency lists included), `/featured`, `/tags`, `sitemap.ts`, `/api/stats`, `/api/tags/[slug]`, the OG image routes except the monthly-rankings one |
