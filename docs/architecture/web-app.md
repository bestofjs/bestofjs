...
| Source | Surfaces |
| --- | --- |
| PostgreSQL, via `@repo/core` queries wrapped in `"use cache"` | Home page (including monthly rankings), `/trends/*`, `/rankings/monthly` and its OG image, `/projects` and its detail pages (NPM dependency lists included), `/featured`, `/tags`, `sitemap.ts`, `/api/stats`, `/api/tags/[slug]`, and the other OG image routes |
| Static JSON generated by the backend (`@/server/api-remote-json`) | The search palette's `/api/search-data` index |
...
2. It then triggers the web app build on Vercel to warm the prerendered pages and invalidates the remaining tags. The search palette reads `projects.json` remotely, so the web app no longer consumes the copy baked at build time.
...
The daily rebuild remains as a warming pass for prerendered pages. Replacing it with direct GET requests is a separate deployment optimization.
