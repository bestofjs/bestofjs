...
| Source | Surfaces |
| --- | --- |
| PostgreSQL, via `@repo/db` queries wrapped in `"use cache"` | Home page (hot lists, recently added, popular tags, catalog stats), `/trends/*`, `/projects` and its detail pages, the OG image routes |
| Static JSON generated by the backend (`@/server/api`) | `sitemap.ts`, `/featured`, `/timeline`, `/rankings/monthly`, the monthly-rankings home card, NPM dependency lists |
