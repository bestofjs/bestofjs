...
export const linksByProject = createSelector(
  ...
  (links) => {
    const flatList = flatten(Object.values(links)
      .map(link => link.projects.map(project => ({ project, link: link._id })))
    )
    return flatList.reduce((acc, item) => {
      return { ...acc, [item.project]: addUniqueLink(acc[item.project], item.link) }
    }, {})
  }
)
...
export const reviewsByProject = createSelector(
  ...
  (reviews) => {
    return Object.values(reviews)
      ...
      .reduce((acc, item) => {
        return { ...acc, [item.project]: addUniqueLink(acc[item.project], item.review) }
      }, {})
  }
)
